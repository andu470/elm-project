module Model.Date exposing (Date, Month(..), compare, compareMonth, full, month, monthToString, monthsBetween, monthsBetweenMonths, offsetMonths, onlyYear, view, year, checkDate)
...
monthsBetween dA dB =
    ...
    in
        case (d1.month, d2.month) of
            (Nothing, Nothing) -> Just (12*(abs (d2.year - d1.year)))
            (Just m1, Just m2) -> Just (abs ((12 * d2.year + monthToInt m2) - (12 * d1.year + monthToInt m1)))
...
compare (Date d1) (Date d2) =
    if d1.year>d2.year then GT
        else if d1.year<d2.year then LT
            else case (d1.month,d2.month) of
                (Nothing, Nothing) ->  EQ
...
checkDate (Date dat) =
    case dat.month of
       (Just m) -> fromInt (dat.year) ++ " " ++ monthToString m
       (Nothing) -> fromInt dat.year

view : Date -> Html msg
view (Date d) =
    div [] [text (checkDate (Date d))]

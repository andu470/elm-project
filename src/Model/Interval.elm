module Model.Interval exposing (Interval, compare, full, length, oneYear, open, view, withDurationMonths, withDurationYears, checkIntervalEnd, checkIntervalStart, checkIntervalLength)
...
compare (Interval intA) (Interval intB) =
    if (Date.compare intA.start intB.start) == LT then LT
        ...
        else case (intA.end,intB.end) of
           (Nothing, Nothing) -> EQ
...
checkIntervalEnd : Interval -> String
checkIntervalEnd (Interval i) = case i.end of
           (Nothing) -> "Present"
           (Just iEnd) -> Date.checkDate iEnd

checkIntervalStart : Interval -> String
checkIntervalStart (Interval i) = Date.checkDate i.start

checkIntervalLength : Interval -> String
checkIntervalLength (Interval i) = 
    case length (Interval i) of
        (Just inter) -> String.fromInt (Tuple.first inter) ++ " years and " ++ String.fromInt (Tuple.second inter) ++ " months"
        (Nothing) -> ""

view : Interval -> Html msg
view interval =
    div [class "interval"] [
        div [class "interval-start"][text (checkIntervalStart interval)]
        , div [class "interval-end"][text (checkIntervalEnd interval)]
        , div [class "interval-length"][text (checkIntervalLength interval)]
    ]
